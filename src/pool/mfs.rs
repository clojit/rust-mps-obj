...
use ffi::{mps_addr_t, mps_pool_t, mps_pool_create_k, mps_class_mfs};
...
impl MfsPool {
    ...
    pub fn with_arena(arena : VmArena, unit_size : usize) -> Result<Self> {
        ...
        let pool = unsafe {
            ...
            let res = mps_pool_create_k(&mut pool, &arena.innner, mps_class_mfs(), args);
...
mod tests {
    ...
    fn arena_create_and_drop() {
        ...
        println!("Total Size: {:?}", pool.total_size());
        println!("Free Size: {:?}", pool.free_size());
    }
